...
def render_manim_video(code: str, job_id: str) -> str:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if tinytex_path not in env.get("PATH", ""):
...
@router.post("/code-only-generate")
def code_only_generate(req: GenerateRequest):
    """Generate Manim code only, without video rendering (for environments that can't render)."""
    logger.info(f"Received code-only prompt: {req.prompt}")
    ...
    try:
        ...
        # Return code without video rendering
        return {
            "success": True,
            "job_id": job_id,
            "message": f"Code generated for: {req.prompt[:50]}...",
            "code": manim_code,
            "video": {
                "url": None,
                "status": "⚠️ Video rendering skipped (code-only mode)",
                "available": False,
                "thumbnail": None
            },
            "animation": {
                "title": f"Code for: {req.prompt[:30]}...",
                "description": "Manim code generated with AI",
                "duration": "5-10 seconds",
                "quality": req.quality or "Medium (720p)",
        ...
        logger.error(f"Error in code_only_generate: {str(e)}")
        return {
            "success": False,
